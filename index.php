...
<head>
    ...
    <style>
        ...
        #slideshow > div.black_background{
            width:100%;
            height:100%;
            background:#000;
            display: flex;
        ...
        #slideshow > div.black_background > img{
		 ...
		 .marvel-device.iphone-x .sleep2 {
			height: 30px;
			top: 350px;
			right: -3px;
		}
		
		.marvel-device.iphone-x .volume, .marvel-device.iphone-x .volume:before, .marvel-device.iphone-x .volume:after, .marvel-device.iphone-x .sleep2 {
            width: 3px;
            background: #b5b5b5;
            position: absolute;
        }
        ...
        .iphone-x .screen:before{background: #333 !important;}
		.iphone-x .screen:after{background: transparent !important;}
...
<body>
                                        ...
                                        <div class="screen">
                                            <div id="slideshow">
                                                ...
                                                while ($details = $res->fetch_assoc()) {
                                                    echo "<div class='black_background'><img src=uploads/{$logo}></div>";
                                                    echo "<div><img src=uploads/{$details['content']}></div>";
                                                }
                                                ?>

                                                <span id="special">
                                                    <h1 id="container_timer">00</h1>
                                                </span>
                                            </div>
                                        </div>
                                        <div class="home"></div>
        ...
        <div class="modal-dialog modal-full" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body p-4" id="result">
                    <div class="row">
                        <div class="col">
                            <img src="img/09a26f7b-4dfa-43dd-8de3-ed0302150ae1.jpg" alt="Winner">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col">
    ...
    <script>
        ...
        const CloseCortains = () =>{
            $("#curtain1").animate({width:"100%"},2000);
            $("#curtain2").animate({width:"100%"},2000);
            $("#contains_logo").fadeIn(1500);
            StopSoundPages();
        }
        ...
        $('#RaffleModal').on('show.bs.modal', function() {
            ...
            $("#container_timer").text("3");
            $("#container_timer").css("font-size", "90px");
            timer = setInterval("RunTimer()",1000);
            seconds = 3;
        });

        //WHERE CLOSE MODAL TRIGGER EVENT
        $('#RaffleModal').on('hide.bs.modal', function() {
            StopSoundPages();
            seconds = 3;
        });
        ...
        const RunTimer = () => {
            $("#container_timer").text(seconds); // this is the same as $("div_timer").html(timer) in       jquery.

            if(seconds < 1) {
                $("#special").css('background-color', 'rgba(255, 255, 255, .0)');
                //$("#container_timer").css("font-size", "30px");
				$("#container_timer").text("");
                clearInterval(timer);
                clearInterval(interval);
                PlayRaffle();
                PlaySoundPages();
        ...
        const PlayRaffle = (stop) => {
            $("#WinnerImage").attr('src','');
            $("#slideshow > div:gt(0)").hide();
            
            interval = setInterval(function() {
                $('#slideshow > div:first')
                    .hide().next().fadeIn().end().appendTo('#slideshow');
                }, 500);
        }
            ...
            $.ajax({
                ...
                success: function(res){
                    if(res){
                        ...
                        $("#WinnerImage").css({width: '40%', height: '100%'});
                        StopSoundPages();
                        PlaySoundWinner();
                        Coffresi();
                        setTimeout(function() {
